...
export const useThemeHeaderHeight = () => {
  ...
  return 0; // 无背景图时使用 React Navigation 的默认 header，不需要额外 padding
};
...
const ThemeHeader: React.FC<ThemeHeaderProps> = ({
  ...
  const defaultBackButton = canGoBack ? (
    ...
    >
      <Svg width={hasBackgroundImage ? 22 : 28} height={hasBackgroundImage ? 22 : 28} viewBox="0 0 24 24" fill="none">
        <Path
          d="M15 18L9 12L15 6"
          stroke={hasBackgroundImage ? '#333' : theme.headerTint}
          strokeWidth={2.5}
          strokeLinecap="round"
  ...
  if (hasBackgroundImage) {
    // 和 UserProfileScreen 完全一样的方式：
    // 背景图作为 ImageBackground 铺满，backgroundColor 作为图片加载前的兜底色
    const totalHeight = statusBarHeight + NAV_BAR_HEIGHT;
    return (
      <View style={[styles.floatingHeader, {height: totalHeight, backgroundColor: theme.headerBackground}]}>
        <ImageBackground
...
const styles = StyleSheet.create({
  // 有背景图时的浮动 header 容器（和 UserProfileScreen 一样的方式）
  floatingHeader: {
    width: '100%',
    zIndex: 10,
  },
  ...
  // 有背景图时的圆角半透明按钮（参照 UserProfileScreen topBarButton）
  themedButton: {
